...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendBookingConfirmationEmail(EmailDetails emailDetails) {
        try {
            ...
            context.setVariable("tenantName", emailDetails.tenantName() != null ? emailDetails.tenantName() : "Celestix");
            context.setVariable("tenantLogoUrl", emailDetails.tenantLogoUrl());

            String htmlContent = templateEngine.process("booking-confirmation", context);

            helper.setTo(emailDetails.customerEmail());
            helper.setFrom(mailFrom);
            String subjectTenantName = emailDetails.tenantName() != null ? emailDetails.tenantName() : "Celestix";
            helper.setSubject("Your " + subjectTenantName + " Booking Confirmation for " + emailDetails.movieTitle());
    ...
    @Override
    @Async
    public void sendOtpEmail(String to, String otp, String tenantName, String tenantLogoUrl) {
        try {
            MimeMessage mimeMessage = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, MimeMessageHelper.MULTIPART_MODE_MIXED_RELATED, "UTF-8");
            ...
            context.setVariable("tenantName", tenantName != null ? tenantName : "Celestix");
            context.setVariable("tenantLogoUrl", tenantLogoUrl);

            String htmlContent = templateEngine.process("password-reset-otp", context);

            helper.setTo(to);
            helper.setFrom(mailFrom);
            String subjectTenantName = tenantName != null ? tenantName : "Celestix";
            helper.setSubject("Your " + subjectTenantName + " Password Reset Code");
            helper.setText(htmlContent, true);
    ...
    @Override
    @Async
    public void sendRefundStatusEmail(String to, String customerName, String bookingId, String status, String tenantName, String tenantLogoUrl) {
        try {
            MimeMessage mimeMessage = mailSender.createMimeMessage();
            ...
            helper.setFrom(mailFrom);
            String subjectTenantName = tenantName != null ? tenantName : "Celestix";
            helper.setSubject("Update on Your " + subjectTenantName + " Refund Request");
            helper.setText(htmlContent, true);
